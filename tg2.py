    ...
    async def _init_db(self):
        async with aiosqlite.connect(self.db_path) as db:
            ...
            await db.execute(
                ...
                "(id INTEGER PRIMARY KEY AUTOINCREMENT, user_id INTEGER, info TEXT, created_at DATETIME DEFAULT CURRENT_TIMESTAMP)"
            )
            await db.commit()
...
async def save_final_result_sql(user_id: int, data: dict):
    ...
    async with aiosqlite.connect("bot_database.db") as db:
        await db.execute(
            "INSERT INTO test_results (user_id, info, created_at) VALUES (?, ?, ?)",
            (user_id, json.dumps(attempt_info, ensure_ascii=False), now)
        )
        await db.commit()
...
async def export_data(message: types.Message):
    ...
    async with aiosqlite.connect("bot_database.db") as db:
        # 1. Достаем тех, кто ПРОШЕЛ полностью (из test_results)
        async with db.execute("SELECT user_id, info, created_at FROM test_results") as cursor:
            finished_rows = await cursor.fetchall()

        # 2. Достаем тех, кто В ПРОЦЕССЕ (из fsm_data)
        async with db.execute("SELECT key, data FROM fsm_data") as cursor:
            unfinished_rows = await cursor.fetchall()

    final_data = []

    # Обрабатываем завершенные тесты
    for row in finished_rows:
        u_id, info_json, timestamp = row
        item = json.loads(info_json)
        item['user_id'] = u_id
        item['Статус'] = "✅ Завершен"
        item['Дата/Время'] = timestamp
        final_data.append(item)

    # Обрабатываем тех, кто не дошел (черновики)
    # Ключ в fsm_data выглядит как "chat_id:user_id"
    finished_ids = [r[0] for r in finished_rows] # Список ID тех, кто уже в архиве

    for row in unfinished_rows:
        key, data_json = row
        user_id = int(key.split(":")[1])
        
        # Если этот юзер уже есть в "завершенных", не дублируем его из черновика
        if user_id in finished_ids:
            continue
            
        data = json.loads(data_json)
        # Если в черновике нет даже имени, возможно, юзер просто нажал /start и ушел
        ...
        item = {
            ...
            'city': data.get('city', '—'),
            'phone': data.get('phone', '—'),
            'score': f"В процессе ({data.get('current_q', 0)}/10)",
            'Статус': "⏳ Не дошел",
            'Дата/Время': "—"
        }
        final_data.append(item)
    ...
    if not final_data:
        await message.answer("База абсолютно пуста.")
        return
    ...
    await message.answer_document(FSInputFile(file_name), caption="📊 Полная выгрузка (Все пользователи)")
    os.remove(file_name)
